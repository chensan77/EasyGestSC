...
Namespace Linq.Dynamic
    ...
    Public Module DynamicExpression
        ...
        Public Function ParseLambda(ByVal parameters() As ParameterExpression, ByVal resultType As Type, ByVal expressionStr As String, ByVal ParamArray values() As Object) As LambdaExpression
            ...
            parser = New ExpressionParser(parameters, expressionStr, values)
    ...
    Friend Class Signature : Implements IEquatable(Of Signature)
        ...
        Public Overloads Function Equals(ByVal other As Signature) As Boolean Implements IEquatable(Of Linq.Dynamic.Signature).Equals
...
amphersand:
        ...
        Function ParseRealLiteral() As Expression
            ...
            If last = "f" Or last = "F" Then
                Dim f As Single
                If Single.TryParse(text.Substring(0, text.Length - 1), System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, f) Then value = f

            Else
                Dim d As Double
                If Double.TryParse(text, System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, d) Then value = d
            End If
        ...
        Function FindPropertyOrField(ByVal type As Type, ByVal memberName As String, ByVal staticAccess As Boolean) As MemberInfo
            Dim flags As BindingFlags = BindingFlags.Public Or BindingFlags.DeclaredOnly Or BindingFlags.NonPublic Or _
        ...
        Shared Function ParseNumber(ByVal text As String, ByVal type As Type) As Object
            Select Case type.GetTypeCode(GetNonNullableType(type))
                ...
                Case TypeCode.UInt64
                    Dim ul As ULong
                    If ULong.TryParse(text, System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, ul) Then Return ul
                Case TypeCode.Single
                    Dim f As Single
                    If Single.TryParse(text, System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, f) Then Return f
                Case TypeCode.Double
                    Dim d As Double
                    If Double.TryParse(text, System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, d) Then Return d
                Case TypeCode.Decimal
                    Dim e As Decimal
                    If Decimal.TryParse(text, System.Globalization.NumberStyles.Number, System.Threading.Thread.CurrentThread.CurrentCulture, e) Then Return e
            End Select
            Return Nothing
        End Function
